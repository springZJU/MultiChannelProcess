function [amplifier_data, frequency_parameters, board_dig_in_data] = RHD_TDT_Merge(BLOCKPATH, DATAPATHs, MERGEFILE, fs)
narginchk(3, 4);
if nargin < 4
    fs = 30000;
end
if iscell(DATAPATHs)
    DATAPATHs = string(DATAPATHs);
end
...
timer = 1;
if length(BLOCKPATH) ~= length(DATAPATHs)
    error("BLOCKPATH should have same length with DATAPATH");
end
fidOut = fopen(MERGEFILE, 'wb');
for bIndex = 1 : length(BLOCKPATH)
    DATAPATH = DATAPATHs(bIndex);
    if ~contains(DATAPATH, 'rhd')
        DATAPATH = char(dirItem(DATAPATH, "rhd", "folderOrFile", "file"));
    else
        DATAPATH = char(DATAPATH);
    end

    fidOutSingle = fopen(strcat(getRootDirPath(DATAPATH(1, :), 1), "rawWave.dat"), 'wb');
    segPoint(bIndex) = timer/fs;
    if ~contains(DATAPATH(1, :), '.rhd')
        error('invalid file type, a *.rhd files are required!');
    end

    for dIndex = 1 : size(DATAPATH, 1)

        [path, file] = ...
            fileparts(DATAPATH(dIndex, :));
        ...
        if (data_present)
            ...
            for i=1:num_data_blocks
                ...
                if (num_amplifier_channels > 0)
                    ...
                    data = reshape(0.195 * (data - 32768), [], 1); % units = microvolts
                    fwrite(fidOut, data, 'integer*2');
        ...
        % Close data file.
        fclose(fid);
        timer = timer + num_amplifier_samples;
        ...
        board_dig_in_data(all(board_dig_in_data == 0, 2), :) = [];
        tempTTL{1, dIndex} = board_dig_in_data;
    end
    fclose(fidOutSingle);
    board_dig_in_data = cell2mat(tempTTL);
    save(strcat(getRootDirPath(DATAPATH(1, :), 1), "TTL.mat"), "board_dig_in_data", "tempTTL", "-v7.3");
end
clearvars -except fidOut BLOCKPATH DATAPATHs MERGEFILE bIndex timer segPoint
fclose(fidOut);
