...
MonkeyName = "cm"; % "cm", "ddz"
ExcludeShank = [{[""]}, {["A44R31"]}]; % First for AC Shank, Second for MGB Shank
ArtificialSortChoose = true;
sigTestMethod = "ttest2"; % "ttest2", "ranksum"
...
%%
    ...
    for SettingParamIdx = 1 : numel(SettingParams)
        ...
        ACIdx = find(strcmp(string({PsthCSITemp.PsthCSIData.Area}), "AC")' & ...
            ~ismember(string({PsthCSITemp.PsthCSIData.Position})', ExcludeShank{1}));
        ...
        MGBIdx = find(strcmp(string({PsthCSITemp.PsthCSIData.Area}), "MGB")' & ...
            ~ismember(string({PsthCSITemp.PsthCSIData.Position})', ExcludeShank{2}));
        ...
        if strcmp(sigTestMethod, "ttest2")
            [h_Click, pValue_Click] = ttest2(ClickFFTAmpData_AC, ClickFFTAmpData_MGB, "Vartype", "unequal"); 
        elseif strcmp(sigTestMethod, "ranksum")
            [pValue_Click, h_Click] = ranksum(ClickFFTAmpData_AC, ClickFFTAmpData_MGB); 
        end
        ...
        if strcmp(sigTestMethod, "ttest2")
            [h_Clicktrain, pValue_Clicktrain] = ttest2(ClickTrainFFTAmpData_AC, ClickTrainFFTAmpData_MGB, "Vartype", "unequal");
        elseif strcmp(sigTestMethod, "ranksum")
            [pValue_Clicktrain, h_Clicktrain] = ranksum(ClickTrainFFTAmpData_AC, ClickTrainFFTAmpData_MGB);
        end
        % AC------All Distribution 
        ...
        if strcmp(sigTestMethod, "ttest2")
            [h_CSI, pValue_CSI] = ttest2(CSIData_AC, CSIData_MGB, "Vartype", "unequal");
        elseif strcmp(sigTestMethod, "ranksum")
            [pValue_CSI, h_CSI] = ranksum(CSIData_AC, CSIData_MGB);
        end
        % AC------All Distribution 
    ...
    annotation(Fig, 'textbox', [.6, .9, .1, .1], 'String', strcat("Monkey: ", MonkeyName), 'FontSize', 20, 'EdgeColor', "none", 'FitBoxToText', 'on');
    print(gcf, strcat(DataRootPath, MonkeyName, "_MSTIFig8_AC_MGB_Distribution.jpg"), "-djpeg", "-r200");
    % close;
