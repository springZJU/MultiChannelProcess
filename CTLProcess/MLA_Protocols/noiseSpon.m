function noiseSpon(MATPATH, FIGPATH)
%% Parameter setting
...
for dIndex = 1:length(devType)
    ...
    acgIdx = cellfun(@(x) max([1, find(x<=1/exp(1), 1, "first")]), normLfpACG);
    ...
    try
        load(fullfile(strrep(FIGPATH, regexp(FIGPATH, "\\noiseSpon\\", "match"), "\Noise\"), "sigRes.mat"));
    catch
        load(fullfile(strrep(FIGPATH, regexp(FIGPATH, "\\noiseSpon\\", "match"), "\RNP_Noise\"), "sigRes.mat"));
    end
...
save(SAVENAME, "trialInfo", "trialAll", "spkRes", "sigRes", "-mat");

% lfpRes
SAVENAME = strcat(FIGPATH, "lfpRes.mat");
save(SAVENAME, "trialInfo", "trialAll", "lfpRes", "sigRes", "-mat");
