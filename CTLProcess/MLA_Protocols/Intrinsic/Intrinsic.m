...
%         normLfpACG = cellfun(@(y) mean(cell2mat(cellfun(@(x) mXcorr(x, "binSize", 1, "maxLag", maxLag), num2cell(y, 2), "UniformOutput", false))), changeCellRowNum(trialsToFFT), "UniformOutput", false);
    ...
    normLfpACG = cellfun(@(y) cell2mat(cellfun(@(x) mXcorr(x, "binSize", 1, "maxLag", maxLag), num2cell(y, 2), "UniformOutput", false)), changeCellRowNum(trialsToFFT), "UniformOutput", false);
...
%     chPSTH = cellfun(@(x) calPsth(x, psthPara, 1e3, 'EDGE', Window, 'NTRIAL', sum(tIndex)), spikePlot, "uni", false);
    ...
    lfpRes(dIndex).acgLFP = cell2struct([acgLfpTime'; cellfun(@(x) mean(x, 2), lfpACG', "UniformOutput", false)], ["acgLfpTime", "lfpACG"]);
