function RLA_CSD_2x16(MATPATH, FIGPATH, shank)
...
window = [-100 500];
selWin = [-20 , 150];
LFPDataset.channels = 1:size(LFPDataset.data,1);
WAVEDataset.channels = 1:size(WAVEDataset.data,1);


trialsLFP = selectEcog(LFPDataset, trialAll, "trial onset", window);
trialsWAVE = selectEcog(WAVEDataset, trialAll, "trial onset", window);

trialsLFP = cellfun(@(x) x([shank:2:32],:), trialsLFP, "UniformOutput", false);
trialsWAVE = cellfun(@(x) x([shank:2:32],:), trialsWAVE, "UniformOutput", false);
