function RLA_MSTIAnesthesia(DATAPATH, FIGPATH)
...
MSTIRegParams = RLA_ParseMSTIAnesthesiaParams(protStr);
...
trialAll(1) = [];
orderTemp = [trialAll.devOrdr];
load(fullfile(SoundMatPath, "MMNSequence.mat"));
SoundInfo = RegMMNSequence(SoundMatIdx);
DevOnset = cellfun(@(x, y) SoundInfo(x).Std_Dev_Onset(end) + y, num2cell(orderTemp)', {trialAll.soundOnsetSeq}', "UniformOutput", false);
Std_Dev_Onset = cellfun(@(x, y) SoundInfo(x).Std_Dev_Onset, num2cell(orderTemp)', "UniformOutput", false);
trialAll = addFieldToStruct(trialAll, Std_Dev_Onset, "StdDevOnset");
trialAll = addFieldToStruct(trialAll, DevOnset, "devOnset");
